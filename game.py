...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, platforms, enemies, pucks_group, powerups_group):
        ...
        # Power-up collection
        collected = pygame.sprite.spritecollide(self, powerups_group, True)
        for pu in collected:
...
def main():
    ...
    for idx, lvl in enumerate(levels):
        ...
        for x, y, w, h in lvl['platforms']:
            platforms.add(Platform(x_offset + x, y, w, h))
        for x, y, pat, spd in lvl['enemies']:
            enemies.add(Enemy(x_offset + x, y, pat, spd))
        for x, y in lvl['powerups']:
            if idx == 2:
                powerups.add(HockeyPowerUp(x_offset + x, y))
            elif idx == 0:
                powerups.add(DumbbellPowerUp(x_offset + x, y))
    ...
    while running:
        ...
        # Puck-enemy collisions
        for puck in pucks:
            if pygame.sprite.spritecollide(puck, enemies, True):
                puck.kill()

        # Determine current level
        current_level = max(0, min(player.rect.centerx // SCREEN_WIDTH, len(levels) - 1))
