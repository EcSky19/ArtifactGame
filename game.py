...
def main():
    ...
    try: bg_collegetown = pygame.image.load('Images/collegetown_bg.png').convert()
    except: bg_collegetown = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); bg_collegetown.fill((100,150,100))
    ...
    backgrounds = [pygame.transform.scale(bg, (SCREEN_WIDTH, SCREEN_HEIGHT)) for bg in (bg_high, bg_cornell, bg_collegetown, bg_lynah, bg_gym)]

    # Level definitions with platforms, enemies, powerups, collectibles for all levels
    gap = 50
    levels = [
        {'platforms': [(0,560,SCREEN_WIDTH-gap,40),(200,450,100,20),(400,350,120,20),(650,300,80,20)], 'enemies': [(500,520,150,2),(350,310,80,3)], 'powerups': [(300,520)], 'collectibles': [(350,500,'coin'),(450,400,'sushi'),(550,300,'beer')]},
        {'platforms': [(gap,560,SCREEN_WIDTH-2*gap,40),(100,460,150,20),(300,380,100,20)], 'enemies': [(900,520,100,1)], 'powerups': [], 'collectibles': [(850,500,'coin'),(950,450,'beer')]},
        {'platforms': [(2*gap,560,SCREEN_WIDTH-2*gap,40),(300,470,120,20),(500,420,100,20)], 'enemies': [(2*SCREEN_WIDTH+200,520,100,2)], 'powerups': [(2*SCREEN_WIDTH-200,520)], 'collectibles': [(2*SCREEN_WIDTH+350,500,'sushi'),(2*SCREEN_WIDTH+450,380,'coin')]},
        {'platforms': [(3*gap,560,SCREEN_WIDTH-2*gap,40),(400,430,100,20)], 'enemies': [(3*SCREEN_WIDTH+350,520,120,2)], 'powerups': [], 'collectibles': [(3*SCREEN_WIDTH+450,500,'beer'),(3*SCREEN_WIDTH+550,450,'sushi')]},
        {'platforms': [(4*gap,560,SCREEN_WIDTH-2*gap,40),(200,500,100,20),(600,400,120,20)], 'enemies': [(4*SCREEN_WIDTH+300,520,120,2)], 'powerups': [(4*SCREEN_WIDTH+400,520)], 'collectibles': [(4*SCREEN_WIDTH+350,480,'coin'),(4*SCREEN_WIDTH+550,380,'beer')]}
    ]
    ...
    while running:
        ...
        cam = max(0, min(player.rect.centerx - SCREEN_WIDTH//2, len(levels)*SCREEN_WIDTH - SCREEN_WIDTH))
        ...
        # Score display
        score_surf = font_small.render(f"Score: {score}", True, (255,223,0))
